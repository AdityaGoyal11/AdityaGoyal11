# Hi, I'm Adi 👋

📍 **Sydney, Australia** | 💻 **Software Engineer** | 🧪 **Building, shipping & occasionally killing ideas**

Software Engineering (Honours) graduate from **UNSW Sydney**.

Currently tutoring at UNSW, fast-tracking a **Master of Information Technology**, and spending the rest of my time building things.

I like turning ideas into working products quickly, learning what breaks, and shipping the ones worth keeping.

![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![React Native](https://img.shields.io/badge/-React_Native-61DAFB?style=flat-square&logo=react&logoColor=black)
![Swift](https://img.shields.io/badge/-Swift-FA7343?style=flat-square&logo=swift&logoColor=white)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white)
![Codex](https://img.shields.io/badge/-Codex-121212?style=flat-square&logo=openai&logoColor=white)

## Start Here

- 🎾 **[Sense My Rally](https://github.com/AdityaGoyal11/sense-my-rally)**  
  Local-first tennis video analysis and coaching platform.

- 🌱 **[SIERRA Investments](https://github.com/AdityaGoyal11/SIERRA-Investments)**  
  Serverless AWS platform for processing and querying ESG investment data.

- 🛡️ **[Currency Guard](https://github.com/AdityaGoyal11/currency-guard)**  
  React-based security project.

- 🌐 **[Portfolio](https://github.com/AdityaGoyal11/Portfolio)**  
  My personal developer portfolio.

## Currently Building

### 🎾 Sense My Rally

Upload tennis footage → analyse rallies → turn computer-vision data into useful coaching feedback.

Building around:

`FastAPI` · `OpenCV` · `React Native` · `Expo` · `YOLO` · `ByteTrack` · `MediaPipe`

The philosophy is integration-first: use strong existing CV models rather than rebuilding everything from scratch.

[View project →](https://github.com/AdityaGoyal11/sense-my-rally)

## Shipped / Finished

### 🌱 SIERRA Investments

An event-driven ESG processing platform built around AWS.

`AWS Lambda` · `S3` · `DynamoDB` · `Express` · `Docker` · `Terraform`

[View project →](https://github.com/AdityaGoyal11/SIERRA-Investments)

### 🛡️ Currency Guard

One of my earlier React projects.

[View project →](https://github.com/AdityaGoyal11/currency-guard)

### 🌐 Portfolio

Personal portfolio and web experiments.

[View project →](https://github.com/AdityaGoyal11/Portfolio)

## Sacked 🪦

_No public casualties yet._

## What I'm Doing

- 🧪 **Rapid prototyping** – turning ideas into usable products quickly
- 🤖 **AI-assisted engineering** – exploring how agents change software development
- 🎓 **Teaching** – tutoring software engineering students at UNSW
- 📚 **Studying** – fast-tracking a Master of Information Technology
- 🔨 **Shipping** – building small products instead of keeping ideas in Notes

## Philosophy

> Build it. Use it. Find out if it deserves to exist.

I care more about working software and learning quickly than pretending every project is going to become a company.

Some projects ship.

Some change direction.

Some get sacked.

All of them teach me something.

## Connect

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/aditya-goyal888/)
[![GitHub](https://img.shields.io/badge/-Follow-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/AdityaGoyal11)

### Work

I'm currently open to **part-time software engineering and technology opportunities**.

If you're building something interesting and think I could help, reach out.
